...
module Feed
  class FeedTest < InMemoryRESTestCase
    ...
    def test_adds_tweet_to_feed_on_tweet_posted
      ...
      event_store.publish(
        ::Social::TweetPosted.new(data: { tweet_id: tweet_id, author: "alice", body: "Hello" })
      )
    ...
    def test_returns_tweets_newest_first
      ...
      event_store.publish(
        ::Social::TweetPosted.new(data: { tweet_id: older, author: "alice", body: "first" })
      )
      event_store.publish(
        ::Social::TweetPosted.new(data: { tweet_id: newer, author: "bob", body: "second" })
      )
