...
- `db` — [hanami-db](https://github.com/hanami/hanami-db) (ROM/Sequel) with SQLite; the migration for the event store tables lives in [config/db/migrate](config/db/migrate) and is managed by `hanami db prepare`
...
- `read_models` — in-memory read models ([app/read_models](app/read_models)) rebuilt at boot by replaying all persisted events, then subscribed to new ones; registered as `read_models.catalog` and `read_models.orders`
- `processes` — the `OrderFulfillment` process manager (subscribed only, never replayed — replaying would re-dispatch commands)
- `seeds` — seeds the catalog with a few products, skipped when the replayed catalog already has any
...
Events are persisted to SQLite via `ruby_event_store-sequel`, so carts and orders survive restarts. The event store is the only persistence: read models stay in memory and are rebuilt from the event log when the app boots. The client uses the same mapper as the Rails app's `RailsEventStore::JSONClient` (`Infra::EventStore.default_mapper`), so `BigDecimal` prices and symbol keys round-trip through JSON.
...
- Read models live in a single process; run the server single-process (the default).
