module Ordering
  class AddItemToRefund
    ...
    attr_reader :refund_id, :order_id, :product_id

    alias aggregate_id refund_id

    def initialize(refund_id, order_id, product_id)
      @refund_id = refund_id
      @order_id = order_id
      @product_id = product_id
      valid = UUID.match?(@refund_id) && UUID.match?(@order_id) && UUID.match?(@product_id)
    rescue TypeError
