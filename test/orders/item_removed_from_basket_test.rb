...
module Orders
  class ItemRemovedFromBasketTest < ActiveJob::TestCase
    test 'remove item when quantity > 1' do
      ...
      event_store.publish(ItemRemovedFromBasket.new(data: {order_id: order_id, product_id: product.id}))
    ...
    test 'remove item when there is another item' do
      ...
      event_store.publish(ItemRemovedFromBasket.new(data: {order_id: order_id, product_id: another_product.id}))
