...
module CommandHandlers
  class CreateOrderTest < ActiveSupport::TestCase
    ...
    test 'order is created' do
      ...
      arrange(event_store, [Events::ItemAddedToBasket.new(data: {order_id: aggregate_id, product_id: product_id})])

      act(event_store, Command::CreateOrder.new(order_id: aggregate_id, customer_id: customer_id))

      assert_changes(event_store, [Events::OrderCreated.new(data: {order_id: aggregate_id, order_number: order_number, customer_id: customer_id})])
    end
    ...
    test 'already created order could not be created again' do
      ...
      arrange(event_store, [Events::OrderCreated.new(data: {order_id: aggregate_id, order_number: order_number, customer_id: customer_id})])
    ...
    test 'expired order could not be created' do
      ...
      aggregate_id = SecureRandom.uuid
      customer_id = 1
      arrange(event_store, [Events::OrderExpired.new(data: {order_id: aggregate_id})])
