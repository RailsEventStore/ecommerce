...
module CommandHandlers
  class CreateOrderTest < ActiveSupport::TestCase
    ...
    test 'order is created' do
      ...
      assert_changes(event_store, [Events::OrderCreated.new(order_id: aggregate_id, order_number: order_number, customer_id: customer_id)])
    end
    ...
    test 'expired order could not be created' do
      ...
      aggregate_id = SecureRandom.uuid
      customer_id = 1
      arrange(event_store, [Events::OrderExpired.new(order_id: aggregate_id)])
